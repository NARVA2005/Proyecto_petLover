        ...
        $mysql = new MYSQL();          
            ...
            if (
                isset($_POST['descripcion']) && !empty($_POST['descripcion'] &&
                    isset($_POST['Stock'])) && !empty($_POST['Stock'] && $_POST['Stock'] > 0 )
            ) {
            ...
            $mysql->desconectar();
                ...
                if (move_uploaded_file($temp, $root . $image)) {
                    ...
                    $root = substr($root,3,strlen($root));
                    $rutaCompleta = $root . $image;
                    $mysql->conectar();
                    $mysql->efectuarConsulta("UPDATE petlover.producto SET descripcion='$nameProduct', stock= $Stock, precio=$precio, imagenProducto = '$rutaCompleta' WHERE id= $id");
                    $mysql->desconectar();
                    loadSuccess();
                        ...
                        try {
                            ...
                            $root = substr($root,3,strlen($root)); // Obtener el nombre del archivo de la URL
                            ...
                            $mysql->conectar();
                            $mysql->efectuarConsulta("UPDATE petlover.producto SET descripcion='$nameProduct', stock=$Stock, precio=$precio, imagenProducto = '$save_path' WHERE id=$id");
                            $mysql->desconectar();
                            loadSuccess();
                        } catch (Exception $ex) {
                            ?>
